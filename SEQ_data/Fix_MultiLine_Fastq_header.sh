...
##It assumes that the fastq file has 6 lines characterizing each sequence (header, 2 sequence, id, 2 quality)
...
sed "1~6 s/^.*/&\;$line\/1/" ${line}_1.fastq | sed "1~6 s/ /\-/g" > ${out_dir}${line}_1.fastq

sed "1~6 s/^.*/&\;$line\/2/" ${line}_2.fastq | sed "1~6 s/ /\-/g" > ${out_dir}${line}_2.fastq 
